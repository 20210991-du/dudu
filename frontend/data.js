...
function makeContribution(status, dominant) {
  ...
  // anomaly: dominant 70-88, second 8-18, third 3-10
  // warn: dominant 55-72, second 15-25, third 8-15
  ...
  // normalize so they sum <100 comfortably
  return [
...
const EQUIPMENT = (() => {
  ...
  for (let i = 0; i < 55; i++) {
    ...
    // Minutes ago for updatedAt (1 hr cycle; stagger around 0-59)
    const minAgo = randInt(1, 59);
    const hh = 14 - Math.floor(minAgo / 60);
    ...
    out.push({
      ...
      threshold: 0.409,
      updatedAt,
      nextCollectMin: 60 - minAgo,  // next collection in X minutes
...
const AI_ANOMALIES = [
  ...
  { node: "TB24-5JN089", label: "희생전류 저하", mse: 0.791, threshold: 0.409, zone: "제3구역",
    contribution: [{sensor:"희생전류",pct:78},{sensor:"방식전위",pct:14},{sensor:"온도",pct:8}],
    summary: "희생양극 방식 전류가 0.4mA로 사실상 기능 상실. 양극 소모 완료 가능성 높음.",
    action: "희생양극 교체 점검 · 접속부 저항 측정 · 교체 주기 재산정" },
  { node: "TB24-5JN117", label: "방식전위 이탈", mse: 0.712, threshold: 0.409, zone: "제2구역",
    contribution: [{sensor:"방식전위",pct:75},{sensor:"AC유입",pct:17},{sensor:"습도",pct:8}],
    summary: "-850mV 방식 기준선을 24시간 중 5회 이탈. 방식 효율 저하 구간으로 판단.",
    action: "심정 조사 · 접지 저항 측정 · 정류기 출력 재조정" },
  { node: "TB24-5JN063", label: "AC 유입 과다", mse: 0.684, threshold: 0.409, zone: "제3구역",
    contribution: [{sensor:"AC유입",pct:81},{sensor:"방식전위",pct:13},{sensor:"희생전류",pct:6}],
...
const AI_WATCH = [
  { node: "TB24-5JN115", label: "성능 저하 트렌드", mse: 0.372, threshold: 0.409, zone: "제4구역",
    contribution: [{sensor:"방식전위",pct:62},{sensor:"온도",pct:22},{sensor:"습도",pct:10}],
    summary: "지난 7일간 MSE가 0.21 → 0.37로 완만히 상승. 아직 임계치 이내.",
    action: "RF 환경 조사 · 향후 48시간 집중 모니터링" },
  { node: "TB24-5JN204", label: "간헐적 통신 지연", mse: 0.351, threshold: 0.409, zone: "제2구역",
  ...
  { node: "TB24-5JN078", label: "습도 상승", mse: 0.342, threshold: 0.409, zone: "제1구역",
...
const AI_INSIGHTS = [
  { kind: "alert", icon: "alert", title: "즉각 조치 권고",
    body: "TB24-5JN042에서 AC 유입 전압이 지속 상승 중. 절연 파손 가능성 있음. 현장 점검 필요." },
  { kind: "trend", icon: "trend", title: "트렌드 예측",
    body: "제2구역 방식전위가 임계값에 근접. 48시간 내 경고 수준 도달 확률 78%." },
  { kind: "suggest", icon: "sparkle", title: "조치 제안",
...
window.APP_DATA = { EQUIPMENT, MAP_MARKERS, AI_ANOMALIES, AI_WATCH, AI_INSIGHTS, LOG_TEMPLATES, FACILITIES };
