...
def predict_device_window(
    ...
    # 2. 피처별 MSE 계산 추가 (시간 축인 axis=1에 대해서만 평균을 냄)
    # last_seq shape: (1, time_steps, n_features)
    # feature_mse shape: (n_features,)
    feature_mse_array = np.mean(np.power(last_seq - pred, 2), axis=1)[0]

    # 3. 피처명과 매핑하여 딕셔너리로 생성
    ...
    return {
        ...
        'feature_contributions': feature_contributions  # 피처별 오차값 추가
    }


# ============================================================
# 5-1. 시각화 (Visualization)
# ============================================================
...
def set_korean_font():
    ...
    if system_name == 'Windows':
        ...
        plt.rc('font', family='NanumGothic')  # 리눅스 환경(Colab 등)에서는 나눔고딕 필요

    # 마이너스 기호 깨짐 방지
    plt.rcParams['axes.unicode_minus'] = False


def plot_device_features(df: pd.DataFrame, device_id: str, features: List[str], save_path: Optional[str] = None,
                         show_plot: bool = True):
    ...
    for ax, feature in zip(axes, features):
        ax.plot(device_df['측정시각'], device_df[feature], label=feature, linewidth=1.5)
        ax.set_ylabel(feature, fontsize=12)
        ax.legend(loc='upper right')
        ax.grid(True, linestyle='--', alpha=0.6)

    axes[-1].set_xlabel('측정시각', fontsize=12)
    fig.suptitle(f'[{device_id}] 장비 센서 데이터 시계열 추이', fontsize=16, fontweight='bold', y=0.98)
    ...
    if show_plot:
        ...
        plt.close(fig)  # 화면에 띄우지 않고 메모리에서만 삭제 (여러 개 저장 시 필수)
    # ------------------------------------
...
def main():
    ...
    # ---- [추가된 시각화 실행 부분] ----
    # ---- [수정된 시각화 실행 부분] ----
    print('\n[6] 모든 장비 데이터 시각화 및 저장 중...')
    if thresholds:
        plot_dir = os.path.join(save_dir, 'plots')  # plots 폴더 안에 모아서 저장
        ...
        for device_id in thresholds.keys():
            ...
            plot_device_features(
                ...
                show_plot=False  # 창을 띄우지 않고 배경에서 저장만 반복!
            )
        print(f'총 {len(thresholds)}개 장비의 그래프 저장이 완료되었습니다. (경로: {plot_dir})')
    else:
    ...
    if thresholds:
        print('  * plots/ 폴더 내에 모든 기기의 시각화 이미지가 저장되었습니다.')
