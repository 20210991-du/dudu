...
def main():
    ...
    if not excel_path.exists():
        raise FileNotFoundError(
            ...
            '예측 대상 데이터 파일을 같은 폴더에 두세요.'
        )

    try:
        from gas_common_model_train import build_master_dataset
        print('\n[2] 예측 대상 데이터 로드 중...')
        target_df = build_master_dataset(str(excel_path))
        ...
        raise ImportError(
            'gas_common_model_train.py 파일이 같은 폴더에 없습니다.\n'
            '두 파일을 같은 디렉터리에 두거나, 직접 DataFrame을 구성해 run_batch_prediction()을 호출하세요.'
        )
